...
async function seedDatabase() {
  ...
  try {
    ...
    // 2. SEED TERMS (TermNodes.json)
    const termNodes = loadJSON('TermNodes.json');
    const formattedTerms = termNodes.map((t: any) => ({
      id: t.id,
      label: t.data.label,
      position: t.position,
      slots: t.data.slots,
    ...
    // Add a "TE" (Technical Elective) term so Foreign Keys don't break
    formattedTerms.push({ id: 'TE', label: 'Technical Electives' });

    const { error: termError } = await supabase.from('terms').insert(formattedTerms);
    if (termError) throw termError;
    console.log('Terms seeded');

    // 3. SEED COURSES (CourseNodes.json)
    const courseNodes = loadJSON('CourseNodes.json');
    const formattedCourses = courseNodes.map((c: any) => ({
        id: c.id,
        // If parentId is "TE" or empty, set to null. Otherwise use the ID.
        parent_id: (c.parentId === "TE" || !c.parentId) ? null : c.parentId, 
        label: c.data.label,
        title: c.data.title,
        credits: c.data.credits,
        prerequisites: c.data.prerequisites,
        antirequisites: c.data.antirequisites
    }));

    const { error: courseError } = await supabase.from('courses').insert(formattedCourses);
    if (courseError) throw courseError;
    console.log('Courses seeded');

    // 4. SEED PROGRAMS (Major&Programs.json)
    const programsData = loadJSON('Major&Programs.json');
    const formattedPrograms = programsData.map((p: any) => ({
      label: p.label,
      total_units: p.totalUnits,
      cse_count: p.cseCount,
      te_count: p.teCount,
      required_courses: p.requiredCourses,
      technical_electives: p.technicalElectives
    }));

    const { error: progError } = await supabase.from('programs').insert(formattedPrograms);
